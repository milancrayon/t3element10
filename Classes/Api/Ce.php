...
class Ce extends \Nng\Nnrestapi\Api\AbstractApi
{
     ...
     * elementsRepository
     *
     * @var \Crayon\T3element\Domain\Repository\ElementsRepository
    ...
    public function fileUpdation()
    {
        ...
        if($data['content']){
            if(sizeof($data['content']) > 0){
                ...
                if($fileswritten == sizeof($data['content']) ){
                    $cacheManager =  GeneralUtility::makeInstance(\TYPO3\CMS\Extbase\Object\ObjectManager::class)->get(\TYPO3\CMS\Core\Cache\CacheManager::class);
